...
export function isSupportedExtApiVersion(version: number): boolean {
  return (
    ...
    version >= MIN_SUPPORTED_EXT_API_VERSION &&
...
export function signTenantToken(
  ...
  const full: TenantContextClaims = {
    ...claims,
    iat: now,
    exp: now + ttlSeconds,
  };
 ...
 * verifyTenantToken.
 */
export function decodeTokenUnverified(
  token: string,
): Partial<TenantContextClaims> | null {
...
export function verifyTenantToken(
  ...
  if (opts.expectedAppSlug && claims.appSlug !== opts.expectedAppSlug) {
    throw new Error(
      `Tenant token issued for app "${claims.appSlug}", not "${opts.expectedAppSlug}"`,
    );
  }
  return claims;
}
...
export function tenantContextMiddleware(options: {
  ...
  return (req: MinimalRequest, res: MinimalResponse, next: NextFn) => {
    ...
    try {
      req.tenantContext = verifyTenantToken(
        String(token || ""),
        options.secret,
        {
...
export function hasScope(
  ...
  return (
    !!claims && Array.isArray(claims.scopes) && claims.scopes.includes(scope)
  );
}
...
export class CoreClient {
  constructor(private readonly opts: { coreApiUrl: string; token: string }) {}
  ...
  private headers(): Record<string, string> {
    return {
      ...
      [TENANT_TOKEN_HEADER]: this.opts.token,
    };
  }
  ...
  async records(
    ...
    const res = await fetch(
      `${this.base()}/records/${schemaSlug}${q ? `?${q}` : ""}`,
      {
  ...
  async recordsBatch(
    ...
    return (await res.json().catch(() => ({}))) as Record<
      string,
